...
impl OllamaManager {
    ...
    pub async fn check_health(&self) -> Result<(), PhazeError> {
        self.ollama
            ...
            .map(|_| ())
            .map_err(|e| {
                PhazeError::Llm(format!(
                    "Ollama is not responding at {}. Is it running at that address? Error: {}",
                    self._base_url, e
                ))
            })
    }
    ...
    pub async fn ensure_phaze_beast(&self) -> Result<(), PhazeError> {
        ...
        self.ensure_model_from_content("phaze-beast", modelfile)
            .await
    }

    /// Provision a model from strings (useful for bundled resources).
    pub async fn ensure_model_from_content(
        &self,
        model_name: &str,
        ...
        let request =
            CreateModelRequest::modelfile(model_name.to_string(), modelfile_content.to_string());
        self.ollama
    ...
    pub async fn ensure_model(
        ...
        self.ensure_model_from_content(model_name, &modelfile_content)
            .await
    }
    ...
    pub async fn ensure_all_phaze_models(&self) -> Result<(), PhazeError> {
        ...
        self.ensure_model_from_content("phaze-coder", MODELFILE_CODER)
            .await?;
        info!("phaze-coder ready");

        // Phaze-Planner: fast planning and analysis
        self.ensure_model_from_content("phaze-planner", MODELFILE_PLANNER)
            .await?;
        info!("phaze-planner ready");

        // Phaze-Reviewer: code review and quality
        self.ensure_model_from_content("phaze-reviewer", MODELFILE_REVIEWER)
            .await?;
