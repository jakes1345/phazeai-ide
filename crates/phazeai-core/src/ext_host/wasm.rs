...
impl WasmExtension {
    pub fn new(
        ...
        let module =
            Module::from_binary(&engine, wasm_bytes).map_err(|e| format!("Invalid WASM: {}", e))?;
        ...
        linker
            .func_wrap(
                "phazeai",
                "log",
                |mut caller: Caller<'_, IdeContext>, ptr: u32, len: u32| {
                    ...
                    let data = mem.data(&caller).get(ptr as usize..end).unwrap_or(&[]);
...
impl Extension for WasmExtension {
    ...
    async fn activate(&mut self, context: IdeContext) -> Result<(), String> {
        ...
        *store.data_mut() = context;
        ...
        if let Ok(activate_fn) = self
            .instance
            .get_typed_func::<(), ()>(&mut *store, "activate")
        {
            activate_fn
                .call(&mut *store, ())
                .map_err(|e| format!("WASM activate error: {}", e))?;
        }
        Ok(())
    }
    ...
    async fn execute_command(&mut self, _command: &str, _args: Value) -> Result<Value, String> {
        ...
        if let Ok(cmd_fn) = self
            .instance
            .get_typed_func::<(), ()>(&mut *store, "execute_command")
        {
            cmd_fn
                .call(&mut *store, ())
                .map_err(|e| format!("WASM command error: {}", e))?;
