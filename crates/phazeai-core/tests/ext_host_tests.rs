...
async fn test_js_extension_execution() {
    ...
    let mut ext = JsExtension::new("test-ext", vec!["test.add".to_string()], js_code).expect("Failed to create JS extension");
    
    let msgs = Arc::new(Mutex::new(Vec::new()));
    let ctx = IdeContext::new(Arc::new(TestDelegate { messages: msgs.clone() }));
    
    // Activate the extension
    ext.activate(ctx).await.expect("Failed to activate JS extension");
    
    // Execute a command
    let res = ext.execute_command(
        "test.add", 
        serde_json::json!({"a": 5, "b": 10})
...
async fn test_vsix_loader() {
    ...
    // Load the vsix
    VsixLoader::load_vsix(&vsix_path, &manager).await.expect("Failed to load vsix");
    
    // Execute the registered command
    let res = manager.execute_command(
        "phazeai.testCommand",
        serde_json::json!({"test": "data"})
    ).await.expect("Failed to execute command from VSIX");
...
async fn test_wasm_extension() {
    ...
    let mut ext = phazeai_core::ext_host::wasm::WasmExtension::new("test-wasm", vec!["dummy".to_string()], &wasm_bytes)
    ...
    // Test activate
    ext.activate(ctx.clone()).await.expect("Failed to activate WASM extension");
    
    // Check if the log message was recorded
    tokio::time::sleep(std::time::Duration::from_millis(50)).await;
    let m = msgs.lock().unwrap();
    assert!(m.iter().any(|msg| msg.contains("WASM Extension Activated!")));
    drop(m);
    
    // Test command
    let res = ext.execute_command("dummy", serde_json::Value::Null).await.expect("Failed to execute WASM command");
    assert_eq!(res, serde_json::Value::Null);
    
    tokio::time::sleep(std::time::Duration::from_millis(50)).await;
    let m = msgs.lock().unwrap();
    assert!(m.iter().any(|msg| msg.contains("WASM Extension Executed Command!")));
}
